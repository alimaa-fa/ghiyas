...
fun App() {
    ...
    Div(attrs = {
        ...
        style {
            ...
            width(100.percent)
            height(100.vh)
            position(Position.Relative) // قفل کردن منو داخل کادر ۶۰۰ پیکسلی
            property("overflow", "hidden") // جلوگیری از بیرون زدن انیمیشن‌های داخلی
        ...
        Div(attrs = {
            ...
            when (currentScreen) {
                "main" -> {
                    ...
                    // نوار تب‌های داشبورد اصلی (همراه با کلاس مخفی کردن اسکرول بار)
                    Div(attrs = { 
                        classes(AppStyleSheet.tabContainer)
                        ...
                        Div(attrs = {
                            classes(AppStyleSheet.tabItem, if (currentMainTab == "irrigation_calendar") AppStyleSheet.tabActive else AppStyleSheet.tabInactive)
                            onClick { currentMainTab = "irrigation_calendar" }
                        }) { Text("تقویم آبیاری") }
                    }
                    ...
                    when (currentMainTab) {
                        ...
                        "standalone_runner" -> {
                            Div(attrs = { style { padding(32.px); textAlign("center"); color(Color("#757575")) } }) {
                                Text("پخش‌کننده فرم پویا (Player) در فازهای بعدی در اینجا قرار می‌گیرد.")
                            }
                        }
                        "irrigation_calendar" -> {
                            Div(attrs = { style { padding(32.px); textAlign("center"); color(Color("#757575")) } }) {
                                Text("تقویم آبیاری (به زودی)")
                            }
                        }
                    }
                }
                ...
                "builder" -> {
                    Div(attrs = { style { padding(32.px); textAlign("center"); color(Color("#757575")) } }) {
                        H2 { Text("بوم گرافیکی محاسبه‌ساز (Builder)") }
                        Text("محیط اتصال بلوک‌ها در فاز ۳ در اینجا پیاده می‌شود.")
                    }
                }
                "profile_manager" -> {
                    Div(attrs = { style { padding(32.px); textAlign("center"); color(Color("#757575")) } }) {
                        Text("مدیریت الگوهای ساخته شده")
                    }
                }
                "backup_restore" -> {
                    Div(attrs = { style { padding(32.px); textAlign("center"); color(Color("#757575")) } }) {
                        Text("محیط خروجی/ورودی JSON")
                    }
                }
            }
        }
